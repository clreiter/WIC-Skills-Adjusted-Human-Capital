...
d1 <- read.csv("./data/DHS_adjustment-factor.csv") %>% 
  select(iso, adj_factor1, adj_factor2) %>% 
...
d3 <- d2 %>% 
  ...
  select(iso, qamys_dhs_full, qamys_dhs_part)

write.csv(d3, "./data/qamys_dhs_full_part.csv", row.names = F)

ggplot(d3, aes(x = qamys_dhs_full, y = qamys_dhs_part)) +
  geom_point() +
  geom_abline(intercept = 0) +
  ylim(0,10) +
  xlim(0,10)
